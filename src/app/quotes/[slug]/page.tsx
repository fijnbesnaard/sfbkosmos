...
export async function generateStaticParams() {
  const items = await reader.collections.quotes.all();
  return items.map((item) => ({ slug: item.slug }));
}

export default async function QuotePage({ params }: QuotePageProps) {
  const { slug } = await params;
  const item = await reader.collections.quotes.read(slug);

  if (!item) notFound();

  return (
    <main className="max-w-4xl mx-auto p-8">
      <PrintPreview title={item.title}>
        <header className="mb-12 border-b border-dark/10 dark:border-light/10 pb-8">
          ...
          {item.amount != null && item.amount > 0 && (
            <p className="text-sm text-dark/50 dark:text-light/50 mb-3">
              Amount: €{item.amount}
            </p>
          )}
